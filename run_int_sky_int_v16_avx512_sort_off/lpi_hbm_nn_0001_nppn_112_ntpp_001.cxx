...
begin_initialization
{
  ...
  double nppc               = 832;        // Average number of particles/cell in ea. species
  ...
  double box_size_y        = 40 * ( 0.06 * 120.0 * 1e-4 / 24.0 ) / 24;
  double box_size_z        = 28 * ( 0.06 * 120.0 * 1e-4 / 24.0 ) / 24;
...
//e0                       = e0*sqrt(waist/width); // at entrance (2D Gaussian)
  ...
  double t_stop            = 100*dt + 0.001*dt; // Runtime in 1/wpe
...
//double qi_He             = Np_i/N_i;            // Charge per He macro ion
  ...
  int use_maxwellian_reflux_bc = 0;
